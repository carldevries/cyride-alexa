...
# agency_list calls the NextBus API to request a list of agencies who allow
...
# route_list calls the NextBus API to request a list of available routes
...
# route_config calls the NextBus API to request a list of stops for a given
...
def route_config(agency, route=None, terse=False):
...
# predictions calls the NextBust API to request prediction data for a given
...
def predictions(agency, route, stop_tag, use_short_titles=False):
...
# vehicle_locations calls the NextBus API to request vehicle location data
...
def vehicle_locations(agency, route, last_time=0):
...
# schedule calls the NextBus API to request a schedule for selected stops
...
# messages calls the NextBus API to request messages sent by CyRide. The 
...
def messages(agency, route=None):
